...
def generate_plot(time_series, filename):
    """
        ...
        """
    plt.plot(time_series[0][0,:,0], label='input', color='lightgray', linestyle='--')
    plt.plot(time_series[1][0,:,0], label='target', linestyle='-')
    plt.plot(time_series[2][0,:,0], label='model')
    plt.legend(loc=9)
    plt.xlabel('time [t]')
    plt.ylabel('signal')
